...
public class PlayerInput : MonoBehaviour
{
    ...
    {
        if (!SelectionToggle.IsSelectionEnabled) return;
        ...
        if (Input.GetKeyDown(KeyCode.Mouse0))
        {
            ...
            SoundManager.PlaySFX("HandClick");
        }
    ...
    {
            float width = (Input.mousePosition.x - StartMousePosition.x)/ canvas.scaleFactor;
            float height = (Input.mousePosition.y - StartMousePosition.y) / canvas.scaleFactor;

            SelectionBox.anchoredPosition = StartMousePosition / canvas.scaleFactor + new Vector2(width / 2, height / 2);
        ...
        for (int i = 0; i < SelectionManager.Instance.AvailableUnits.Count; i++)
            ...
            Vector3 unitPosition =
                Camera.WorldToScreenPoint(SelectionManager.Instance.AvailableUnits[i].transform.position) / canvas.scaleFactor;
